...
document.addEventListener("DOMContentLoaded", function() {
    ...
    document.getElementById("toggle-cart").style.display = "none";
        ...
        function toggleFavorite(event, productId) {
            event.stopPropagation(); // Evitar que el evento se propague a elementos padre
            const product = products.find(p => p.id === productId);
            if (product) {
        ...
        // Función para actualizar el panel de favoritos
        function updateFavoritesList() {
            const favoritesContainer = document.getElementById('favorites-items');
            if (!favoritesContainer) return;
            
            if (favoritesList.length === 0) {
                favoritesContainer.innerHTML = '<p class="empty-favorites">No hay productos en favoritos</p>';
                return;
            }
        ...
        function loadFavoritesFromLocalStorage() {
            ...
            if (savedFavorites) {
                ...
                // Actualizar el estado 'favorite' en el array products
                products.forEach(product => {
                    product.favorite = favoritesList.some(fav => fav.id === product.id);
                });
                updateFavoritesList();
            }
        }
        ...
        function toggleFavoritesPanel() {
            document.getElementById('favorites').classList.toggle('open');
            
            // Ocultar el carrito si está abierto cuando abro favoritos
            if (document.getElementById('favorites').classList.contains('open')) {
                document.getElementById('cart').classList.remove('open');
            }
        }
            ...
            productsContainer.innerHTML = filteredProducts.map((product) => 
                ...
                <div class="product-card" data-product-id="${product.id}">
                    ...
                    <div class="card-title">
                        <h3>${product.name}</h3>
                        <div class="product-favorite" data-product-id="${product.id}" onclick="toggleFavorite(event, ${product.id})">
                            <i class="fas fa-heart ${product.favorite ? 'active' : ''}"></i>
                        </div>
                    </div>
                    <div class="product-cantidad">
                        <p id="precios">Precio: ${product.price.toFixed(2)}€</p>
                        <select id="cantidad-${product.id}">
                            <option value="1">1</option>
                            <option value="2">2</option>
                            <option value="3">3</option>
        ...
        // Modificar la función setupProductModal para incluir favoritos en el modal
        ...
        setupProductModal = function() {
                        ...
                        <div class="modal-body">
                            ...
                            <div class="modal-details">
                                ...
                                <div class="modal-actions">
                                    ...
                                    <div class="modal-favorite" onclick="toggleFavorite(event, currentProductId)">
            ...
            let currentProductId = null;
            window.currentProductId = null; // Hacer accesible globalmente
            ...
            function openModal(productId) {
                ...
                currentProductId = productId;
                window.currentProductId = productId; // Actualizar la variable global
                ...
                // Actualizar el estado del icono de favorito
        ...
        // Inicialización para favoritos
        document.addEventListener("DOMContentLoaded", function() {
            // Añadir evento al icono de favoritos
            ...
            // Cerrar panel de favoritos al hacer clic fuera
